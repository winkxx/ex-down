...
header = {
    ...
    'Connection': 'ipb_member_id: 5234081,ipb_pass_hash: 1027beacc8ebd818c2314cb0a64eeacf,igneous: 1b7197677'
}
...
def getHTML(url):
    ...
    while get_html == '':
        try:
            get_html = requests.get(url, headers=header).content.decode('utf-8')
...
def getIMGHTML(url, header_img):
    ...
    while get_html == '':
        try:
            get_html = requests.get(url, headers=header_img).content.decode('utf-8')
        except requests.exceptions.SSLError:
            print('Connection refused by target server...sleep 1s')
            time.sleep(1)
            continue
...
def downloadByPage(g_url, html):
    ...
    return imgUrlGraberAndDownload(page_url_list=page_url_list, title=title, path='D:\\下载文件\\Ex')
...
if __name__ == '__main__':
    ...
    urls = 'https://e-hentai.org/g/2294767/5a495ecacb/'
